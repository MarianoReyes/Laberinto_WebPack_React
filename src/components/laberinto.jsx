...
function Laberinto({ setGano }) {
  ...
  const fetchLaberinto = async () => {
    const fet = `https://maze.juanelcaballo.club/?type=json&w=${ancho}&h=${altura}`;

    const response = await fetch(fet)
      .then((responsepag) => responsepag.json()).then((responseInJSON) => responseInJSON);
  ...
  return (
    ...
    >
      ...
      <div css={{
        color: 'white', textAlign: 'center', width: '100%', display: 'flex', flexGrow: '1',
      }}
      >
        ...
        <div css={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <input
            css={{
              color: '#fff', margin: '5px', backgroundColor: '#0BE345', border: '0px', borderRadius: '5px',
            }}
            placeholder="Altura"
      ...
      >
        ...
        {
                // construccion del laberinto con imagenes en lugar de simbolos
                laberinto.map((row, fila) => (
                  <div css={{ display: 'flex', flexDirection: 'row', justifyContent: 'center' }} key={fila.id}>
                    {
                            row.map((element, col) => {
                              if (element === '-' || element === '|') {
                                return (
                                  <div
                                    css={{
                                      backgroundImage: `url(${pared})`, height: '40px', width: '40px', backgroundSize: 'contain', backgroundRepeat: 'no-repeat',
                                    }}
                                    key={col.id}
                                  />
                                );
                                ...
                                return (
                                  <div
                                    css={{
                                      backgroundImage: `url(${columna})`, height: '40px', width: '40px', backgroundSize: 'contain', backgroundRepeat: 'no-repeat',
                                    }}
                                    key={col.id}
                                  />
                                );
                                ...
                                return (
                                  <div
                                    css={{
                                      backgroundImage: `url(${trofeo})`, height: '40px', width: '40px', backgroundSize: 'contain', backgroundRepeat: 'no-repeat',
                                    }}
                                    key={col.id}
                                  />
                                );
                              ...
                              return (
                                <div css={{ height: '40px', width: '40px', border: '10px' }} key={col.id} />
                              );
                            })
                        }
                  </div>
